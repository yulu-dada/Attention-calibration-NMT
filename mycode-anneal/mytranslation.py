...
class AttackTransformerModel(TransformerModel):
    ...
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return AttackTransformerDecoder(
            args,
            tgt_dict,
            embed_tokens,
...
class AttackTransformerDecoder(TransformerDecoder):
    ...
    def extract_features_scriptable(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            x, layer_attn, _, layer_mask = layer(
                x,
...
class AttackMultiheadAttention(MultiheadAttention):
    ...
    def forward(
        ...
        if loss_type=='nmt':
            attn_weights_float = new_weight*attn_weights_float + (1-new_weight)*torch.mul(attn_weights_float, torch.exp(1-mask))
        elif loss_type=='mask':
